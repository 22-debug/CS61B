...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        int swimIndex = index;
        while (swimIndex > 1 && contents[swimIndex].myPriority < contents[parentIndex(swimIndex)].myPriority) {
            swap(swimIndex, parentIndex(swimIndex));
            swimIndex /= 2;
        }
        return;
    }
    ...
    private void sink(int index) {
        ...
        int sinkIndex = index;
        while (leftIndex(sinkIndex) <= size) {
            int targetIndex = min(leftIndex(sinkIndex), rightIndex(sinkIndex));
